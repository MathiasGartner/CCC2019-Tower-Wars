...
namespace CCC2019SS
{
    ...
    public class Quest
    {
        ...
        public List<Point> pointsForTower { get; set; }
        ...
        {
            pointsForTower = new List<Point>();
            int grid = 5;
            for (int i = 0; i < WX; i += grid)
            {
                for (int j = 0; j < WY; j += grid)
                {
                    if (this.VisitedPoints.Any(p => p.X == i && p.Y == j))
                    {

                    }
                    else
                    {
                        pointsForTower.Add(new Point() { PointsInRange = 0, X = i, Y = j });
                    }
                }
            }

            foreach (var p in pointsForTower)
            {
                foreach (var w in this.VisitedPoints)
                {
                    if (w.IsInRange(p.X, p.Y, Range))
                    {
                        p.PointsInRange++;
                    }
                }
            ...
            int maxTowerToBuy = this.RemainingGold / TowerCost;

            pointsForTower = pointsForTower.OrderByDescending(p => p.PointsInRange).ThenBy(p => p.Y - this.VisitedPoints[0].Y).ThenBy(p => p.X - this.VisitedPoints[0].X).ToList();

            var towerPoints = pointsForTower.Take(maxTowerToBuy);

            this.RemainingGold -= maxTowerToBuy * TowerCost;

            foreach (var t in towerPoints)
            {
                this.Towers.Add(new Tower() { Status = "S", X = t.X, Y = t.Y });
            }
        ...
        public void Fight()
        {
            ...
            while (true)
            {
                ...
                if (i % 100 == 0)
                {
                    System.Console.WriteLine(i + " " + this.Towers.Count);
                }
                var alienstoConsider = this.Aliens.Where(p => !p.IsDead && p.IsSpawned).ToList();
                foreach (var a in alienstoConsider)
                {
                ...
                foreach (var t in this.Towers)
                {
                    if (t.Status == "L")
                    {
                        ...
                        else
                        {
                            ...
                            //}
                            //var nextAlien = alienstoConsider.OrderBy(p => p.towerDistance).ThenBy(p => p.Id).FirstOrDefault();
                ...
                {
                    a.IsDead = true;
                    this.RemainingGold += Loot;
                }

                while(RemainingGold >= this.TowerCost && this.Towers.Count < 500)
                {
                    var point = this.pointsForTower[this.Towers.Count()];
                    this.Towers.Add(new Tower() { X = point.X, Y = point.Y, TimeBuilt = i });
                    this.RemainingGold -= this.TowerCost;
                }
    ...
    class Program
    {
        ...
        {
            var filenames = Enumerable.Range(4, 2).Select(p => "..\\..\\data\\level6_" + p + ".in").ToList();
            ...
            foreach (var filename in filenames)
            {
                ...
                quest.Loot = dataAliensProps[2].AsInt();
                ...
                quest.RemainingGold = lines[6 + quest.NAliens].AsInt();
                ...
                //int[] towercoords = { 1, 3, 5, 6 };
                //for (int t = 0; t < towercoords.Count(); t += 2)
                //{
                //    quest.Towers.Add(new Tower() { X = towercoords[t], Y = towercoords[t + 1] });
                //}

                var towers = quest.Towers.Select(p => p.X + " " + p.Y);
                System.IO.File.WriteAllLines(filename + ".tower", towers);
                var vis = quest.VisitedPoints.Select(p => p.X + " " + p.Y);
                System.IO.File.WriteAllLines(filename + ".vis", vis);
                System.Console.WriteLine("fight!");
                quest.Fight();

                towers = quest.Towers.Select(p => p.X + " " + p.Y);
                System.IO.File.WriteAllLines(filename + ".tower.all", towers);
                vis = quest.VisitedPoints.Select(p => p.X + " " + p.Y);
                System.IO.File.WriteAllLines(filename + ".vis.all", vis);
                System.Console.WriteLine(quest.Won ? "WON" : "LOSS");
                ...
                var s = quest.Towers.Select(p => p.X + " " + p.Y + " " + p.TimeBuilt);
